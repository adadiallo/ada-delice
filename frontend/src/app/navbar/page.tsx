...
export default function NavbarScroll() {
  ...
  return (
    <nav className="flex space-x-4">
      <NavLink id="accueil" activeSection={activeSection}>Accueil</NavLink>
      <NavLink id="formules" activeSection={activeSection}>Formules</NavLink>
      <NavLink id="menus" activeSection={activeSection}>Menus</NavLink>
      <NavLink id="clients" activeSection={activeSection}>Nos Clients</NavLink>
    </nav>
  );
}

function NavLink({ id, activeSection, children }: { id: string; activeSection: string; children: React.ReactNode }) {
  return (
    <a
      href={`#${id}`}
      className={`pb-1 transition-all duration-300 ${
        activeSection === id ? "border-b-2 border-[#F28C28]" : ""
      }`}
    >
      {children}
    </a>
